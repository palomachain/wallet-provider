...
```jsx
import { useWallet } from '@palomachain/wallet-provider';
import React from 'react';
...
<!-- source packages/src/@palomachain/use-wallet/useWallet.ts --pick "Wallet" -->

[packages/src/@palomachain/use-wallet/useWallet.ts](packages/src/@palomachain/use-wallet/useWallet.ts)
...
```jsx
import { useConnectedWallet } from '@palomachain/wallet-provider'
...
```jsx
import { useLCDClient } from '@palomachain/wallet-provider';
...
You can check `src/@palomachain/wallet-provider/` in the Chrome Devtools / Sources Tab, and you can also use breakpoints
here for debug.
...
the `packages/src/@palomachain/wallet-provider/env.ts` file. (or just make an issue is OK)
